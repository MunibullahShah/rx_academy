...
import 'package:rx_academy/data/models/CachedCourse.dart';
...
class CacheManager {
    ...
    if (cache != "") {
      try {
        CachedCourses courses =
            await CachedCourses.fromJson(json.decode(cache));
        return courses;
      } catch (e, s) {
        print(e);
        print(s);
  ...
  Future<bool> isCached(int id) async {
    ...
    CachedCourses courses = await CachedCourses.fromJson(json.decode(cache));
    if (courses.courses.indexWhere((element) => element.id == id) == -1)
      return false;
