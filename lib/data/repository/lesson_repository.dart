...
class LessonRepositoryImpl extends LessonRepository {
  ...
  Future<LessonResponse> getLesson(int courseId, int lessonId) async {
    ...
    try {
      ...
      print(s);
      if (isCached) {
        var course = (await cacheManager.getFromCache())
            .courses
            .firstWhere((element) => element.id == courseId);
        if (course.lessons != null &&
            course.lessons.indexWhere((element) => element.id == lessonId) !=
                -1) {
          response =
              course.lessons.firstWhere((element) => element.id == lessonId);
        } else {
  ...
  @override
  Future<List<LessonResponse>> getAllLessons(
      int courseId, List<int> lessonIds) async {
    List<LessonResponse> lessons = List();
    ...
    await Future.forEach(lessons, (element) async {
      ...
      List<String> urls =
          _getLinkedFileUrls((element as LessonResponse).content);

      for (String webUrl in urls) {
        var file64 = await _getBase64String(webUrl);
        var currentElement = (element as LessonResponse);
        currentElement.content =
            currentElement.content.replaceFirst(webUrl, file64);
